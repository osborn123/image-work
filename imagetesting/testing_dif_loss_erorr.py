...
def save_errors(error_dict, file_path):
...
def main():
    ...
    for criterion_name, criterion in criterion_list.items():
        for model_name, model, optimizer, train_dataloader, test_dataloader in [
            ...
            # 保存模型
            model_file_path = f'saved_models/{model_name}_{criterion_name}_model.pth'
            save_model(model, model_file_path)
            print(f"Saved {model_name} model with {criterion_name} loss to {model_file_path}")

    # 保存误差结果
    error_file_path = 'saved_errors/error_dict.pkl'
    save_errors(error_dict, error_file_path)
    print(f"Saved error results to {error_file_path}")
