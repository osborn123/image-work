...
def main(args):
    ...
    zfnet_model = ZFNet(num_classes=10).to(device)

    # Load pre-trained weights
    resnet_weights_path = 'C:\\Users\\liu18\\image-work\\imagetesting\\pytorch_model.pth'
    zfnet_weights_path = 'C:\\Users\\liu18\\image-work\\imagetesting\\zfnet_final.pth'
    resnet_model.load_state_dict(torch.load(resnet_weights_path, map_location=device))
    zfnet_model.load_state_dict(torch.load(zfnet_weights_path, map_location=device))
